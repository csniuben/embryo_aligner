function handles = bubbleplot3(ax,x,y,z,r,varargin)
...
%set(gcf,'DoubleBuffer','on');grid(ax,'on');

for k=1:numel(x)
    if size(r)==[numel(x) 3]
    [X{k},Y{k},Z{k}]=ellipsoid(ax,x(k),y(k),z(k),r(k,1),r(k,2),r(k,3),N(k));
    else 
    [X{k},Y{k},Z{k}]=ellipsoid(ax,x(k),y(k),z(k),r(k),r(k),r(k),N(k));
    end
    h(k)=surface(ax,X{k},Y{k},Z{k},xargs{:}); hold(ax,'on')
end
